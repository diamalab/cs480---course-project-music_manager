...
public class ArtistServletList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			request.setAttribute("ArtistList", userservice.SQ1());
		...
		request.getRequestDispatcher("/Queryresult/ArtistList.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArtistService userservice = new ArtistService();
		try {
			request.setAttribute("ArtistList", userservice.findall());
		} catch (InstantiationException | IllegalAccessException
				| ClassNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
